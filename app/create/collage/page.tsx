...
type View = 'landing' | 'step1' | 'step2' | 'step3' | 'confirm' | 'generating' | 'result'
...
export default function CollagePage() {
  ...
  // No session state
  if (sessionToken === null && typeof window !== 'undefined' && !localStorage.getItem('menulab_session')) {
    return (
      <div style={{ minHeight: '100vh', background: '#f0eeeb', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        <div style={{ background: '#fff', borderRadius: '20px', padding: '32px 24px', maxWidth: '360px', width: '100%', textAlign: 'center', boxShadow: '0 8px 40px rgba(0,0,0,0.1)' }}>
          <div style={{ fontSize: '36px', marginBottom: '16px' }}>🔐</div>
          <h2 style={{ fontSize: '18px', fontWeight: 900, marginBottom: '8px' }}>로그인이 필요해요</h2>
          <p style={{ color: '#888', fontSize: '14px', marginBottom: '24px', lineHeight: 1.6 }}>모음컷 기능을 사용하려면 로그인해주세요.</p>
          <a href="/api/auth/kakao" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', padding: '15px', background: '#FEE500', color: '#000', borderRadius: '12px', fontSize: '15px', fontWeight: 800, textDecoration: 'none' }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path fillRule="evenodd" clipRule="evenodd" d="M12 3C6.477 3 2 6.582 2 11.012c0 2.782 1.696 5.232 4.27 6.729l-1.088 3.98a.3.3 0 0 0 .46.325l4.603-3.05c.573.08 1.162.122 1.755.122 5.523 0 10-3.582 10-8.012S17.523 3 12 3Z" fill="#000"/></svg>
            카카오로 시작하기
          </a>
        </div>
      </div>
  ...
  if (view === 'landing') {
    return (
      <div style={{ minHeight: '100vh', background: '#f0eeeb', padding: '16px 16px 32px' }}>
        <style>{`@keyframes spin { to { transform: rotate(360deg) } }`}</style>
        <div style={{ maxWidth: '390px', margin: '0 auto' }}>

          <button
            onClick={() => router.push('/mypage')}
            style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: 700, color: '#888', padding: '8px 0', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '4px' }}
          >← 돌아가기</button>

          <div style={{ background: '#fff', borderRadius: '16px', overflow: 'hidden' }}>
            <div style={{ padding: '24px 20px 16px' }}>
              <span style={{ display: 'inline-block', background: 'rgba(196,81,13,0.08)', color: '#FF5722', fontSize: '12px', fontWeight: 700, padding: '4px 12px', borderRadius: '100px', marginBottom: '14px' }}>💎 40젬~ / 세트</span>
              <h2 style={{ fontSize: '26px', fontWeight: 900, color: '#111', letterSpacing: '-0.5px', marginBottom: '8px', lineHeight: 1.25, whiteSpace: 'pre-line' }}>{'여러 메뉴를\n한 컷에'}</h2>
              <p style={{ color: '#888', fontSize: '13px', marginBottom: '20px', lineHeight: 1.6, whiteSpace: 'pre-line' }}>{'업로드한 음식 사진들을 AI가\n자연스럽게 하나의 모음컷으로 만들어드려요.'}</p>

              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px' }}>
                {[
                  { icon: '📸', label: '사진 업로드',  desc: '최대 5장까지 업로드' },
                  { icon: '🎨', label: '배경 선택',    desc: '20+ 프리셋 배경 중 선택' },
                  { icon: '📐', label: '구도 선택',    desc: '항공뷰 · 측면뷰' },
                ].map(item => (
                  <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: '12px', background: '#fff', border: '1px solid rgba(0,0,0,0.08)', borderRadius: '16px', padding: '14px 16px' }}>
                    ...
                    <div>
                      <p style={{ fontWeight: 800, fontSize: '13px', color: '#111', marginBottom: '2px' }}>{item.label}</p>
            ...
            <div style={{ padding: '8px 20px 20px' }}>
              <button
                ...
                style={{ width: '100%', padding: '15px 28px', borderRadius: '100px', background: '#FF5722', color: '#fff', fontWeight: 800, fontSize: '16px', border: 'none', cursor: 'pointer' }}
              >모음컷 만들러 가기 →</button>
            </div>
          </div>
        </div>
  ...
  if (view === 'step1') {
    return (
      <div style={{ minHeight: '100vh', background: '#f0eeeb', padding: '16px 16px 40px' }}>
        <style>{`@keyframes spin { to { transform: rotate(360deg) } }`}</style>
        <div style={{ maxWidth: '480px', margin: '0 auto' }}>

          {/* Header */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
            <button onClick={() => setView('landing')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '20px', color: '#888', padding: '4px' }}>←</button>
            <div>
              <p style={{ fontSize: '10px', color: '#FF5722', fontWeight: 700, letterSpacing: '2px' }}>모음컷 · STEP 1 / 3</p>
              <h2 style={{ fontSize: '18px', fontWeight: 900, color: '#111' }}>사진 업로드</h2>
            </div>
          </div>
          ...
          {photos.length === 0 && (
            <div
              ...
              style={{
                ...
                textAlign: 'center', cursor: 'pointer', background: dragOver ? 'rgba(255,87,34,0.04)' : '#fff',
          ...
          <p style={{ fontSize: '12px', color: '#aaa', marginBottom: '20px', textAlign: 'center' }}>{photos.length}장 선택됨 (최소 2장)</p>
          ...
          <button
            ...
            style={{ width: '100%', padding: '15px', borderRadius: '12px', background: photos.length >= 2 ? '#FF5722' : '#eee', color: photos.length >= 2 ? '#fff' : '#bbb', fontWeight: 800, fontSize: '16px', border: 'none', cursor: photos.length >= 2 ? 'pointer' : 'not-allowed', transition: 'all 0.2s' }}
          >다음 → 배경 선택</button>
        </div>
      </div>
    )
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#f0eeeb', padding: '16px 16px 40px' }}>
        <div style={{ maxWidth: '480px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
            <button onClick={() => setView('step1')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '20px', color: '#888', padding: '4px' }}>←</button>
            <div>
              <p style={{ fontSize: '10px', color: '#FF5722', fontWeight: 700, letterSpacing: '2px' }}>모음컷 · STEP 2 / 3</p>
              <h2 style={{ fontSize: '18px', fontWeight: 900, color: '#111' }}>배경 선택</h2>
            </div>
          </div>
          ...
          <button
            ...
            style={{ width: '100%', padding: '15px', borderRadius: '12px', background: selectedBgId ? '#FF5722' : '#eee', color: selectedBgId ? '#fff' : '#bbb', fontWeight: 800, fontSize: '16px', border: 'none', cursor: selectedBgId ? 'pointer' : 'not-allowed', transition: 'all 0.2s' }}
          >다음 → 구도 선택</button>
        </div>
      </div>
    )
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#f0eeeb', padding: '16px 16px 40px' }}>
        <div style={{ maxWidth: '480px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
            <button onClick={() => setView('step2')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '20px', color: '#888', padding: '4px' }}>←</button>
            <div>
              <p style={{ fontSize: '10px', color: '#FF5722', fontWeight: 700, letterSpacing: '2px' }}>모음컷 · STEP 3 / 3</p>
              <h2 style={{ fontSize: '18px', fontWeight: 900, color: '#111' }}>구도 선택</h2>
            </div>
          </div>
          ...
          <button
            ...
            disabled={!angle}
            style={{ width: '100%', padding: '15px', borderRadius: '12px', background: angle ? '#FF5722' : '#eee', color: angle ? '#fff' : '#bbb', fontWeight: 800, fontSize: '16px', border: 'none', cursor: angle ? 'pointer' : 'not-allowed', transition: 'all 0.2s' }}
          >확인 → 생성하기</button>
        </div>
      </div>
    )
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#f0eeeb', padding: '16px 16px 40px' }}>
        <div style={{ maxWidth: '480px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
            <button onClick={() => setView('step3')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '20px', color: '#888', padding: '4px' }}>←</button>
            <h2 style={{ fontSize: '18px', fontWeight: 900, color: '#111' }}>생성 확인</h2>
          </div>
          ...
          <div style={{ background: '#fff', borderRadius: '16px', padding: '20px', marginBottom: '16px' }}>
            {[
              ...
              <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
          ...
          {error && (
            <div style={{ background: 'rgba(239,68,68,0.07)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: '12px', padding: '12px 16px', marginBottom: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <button
              onClick={() => setView('step3')}
              style={{ flex: 1, padding: '14px', borderRadius: '12px', background: '#fff', border: '1.5px solid rgba(0,0,0,0.1)', color: '#555', fontWeight: 700, fontSize: '14px', cursor: 'pointer' }}
            >← 다시 설정</button>
            <button
              onClick={handleGenerate}
              style={{ flex: 2, padding: '14px', borderRadius: '12px', background: '#FF5722', color: '#fff', fontWeight: 800, fontSize: '15px', border: 'none', cursor: 'pointer' }}
            >이대로 만들기 →</button>
          </div>
        </div>
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#f0eeeb', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ width: '60px', height: '60px', borderRadius: '50%', border: '4px solid rgba(255,87,34,0.2)', borderTopColor: '#FF5722', animation: 'spin 0.9s linear infinite', margin: '0 auto 20px' }} />
      ...
      <div style={{ minHeight: '100vh', background: '#f0eeeb', padding: '16px 16px 40px' }}>
        <div style={{ maxWidth: '480px', margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
            ...
            <h2 style={{ fontSize: '18px', fontWeight: 900, color: '#111' }}>모음컷 완성!</h2>
          </div>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <button
              ...
              style={{ flex: 1, padding: '14px', borderRadius: '12px', background: '#FF5722', color: '#fff', fontWeight: 800, fontSize: '15px', border: 'none', cursor: 'pointer' }}
            >다운로드</button>
            <button
          ...
          <p style={{ textAlign: 'center', fontSize: '12px', color: '#aaa', marginTop: '12px' }}>남은 젬: 💎 {gemBalance}젬</p>
        </div>
      </div>
    )
  }
